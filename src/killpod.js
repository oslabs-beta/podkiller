...
async function killPod() {
  ...
  try {
    ...
    // Additional debug code used to find issue with Null - MOVED INSIDE FUNCTION
    console.log('res exists:', !!res);
    console.log('res.body exists:', !!res.body);
    console.log('res.items exists:', !!res.items);
    console.log('res.items is array:', Array.isArray(res.items));
    console.log('res.items length:', res.items ? res.items.length : 'undefined');

    if (!res || !res.items || !Array.isArray(res.items)) {
      console.log('Validation failed!');
      return 'Invalid response from Kubernetes API';
    }
