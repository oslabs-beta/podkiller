  ...
  statisticsDiv.innerHTML = `
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 15px;">
            <div style="text-align: center; padding: 15px; background: rgba(255, 255, 255, 0.03); border-radius: 6px; border: 1px solid rgba(255, 255, 255, 0.08);">
                <div style="font-size: 1.8em; font-weight: bold; color: #00ff88; margin-bottom: 5px;">${
                  sessionStats.podsKilled
                }</div>
            ...
            <div style="text-align: center; padding: 15px; background: rgba(255, 255, 255, 0.03); border-radius: 6px; border: 1px solid rgba(255, 255, 255, 0.08);">
                <div style="font-size: 1.8em; font-weight: bold; color: #00ff88; margin-bottom: 5px;">${averageRecovery.toFixed(
                  2
                )}s</div>
            ...
            <div style="text-align: center; padding: 15px; background: rgba(255, 255, 255, 0.03); border-radius: 6px; border: 1px solid rgba(255, 255, 255, 0.08);">
                <div style="font-size: 1.8em; font-weight: bold; color: #00ff88; margin-bottom: 5px;">${
                  sessionStats.recoveryTimes.length
                }</div>
            ...
            <div style="text-align: center; padding: 15px; background: rgba(255, 255, 255, 0.03); border-radius: 6px; border: 1px solid rgba(255, 255, 255, 0.08);">
                <div style="font-size: 1.8em; font-weight: bold; color: #ef4444; margin-bottom: 5px;">${
                  sessionStats.failedRecoveries
                }</div>
            ...
            <button onclick="downloadReport()" style="background: rgba(88, 236, 204, 0.2); border: 1px solid rgba(31, 149, 94, 0.3); color: #26e9dfff; padding: 6px 12px; border-radius: 4px; font-size: 0.8em; cursor: pointer;">
                Download Report
            </button>
        </div>
    `;
...
async function downloadReport() {
  try {
    const response = await fetch(
      'http://localhost:3000/api/reports/export?format=csv'
    );
    if (!response.ok) {
...
function setConnectionStatus(connected) {
  ...
  if (connected) {
    ...
    statusText.style =
      'font-family: "Kode Mono", monospace; font-optical-sizing: auto; font-weight: 800; color: #a4f2b9';
    killBtn.disabled = false;
    if (
      !activityLog.some((log) => log.message.includes('Connected to Minikube'))
    ) {
    ...
    statusText.style =
      'font-family: "Kode Mono", monospace; font-optical-sizing: auto; font-weight: 800; color: #f84848ff';
...
async function killRandomPod() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      addLogEntry(
        `Pod killed: ${result.killedPodName || targetPod.name}`,
        'kill'
      );

      if (result.replacementPodName) {
        addLogEntry(
          `Replacement pod created: ${result.replacementPodName}`,
          'replacement'
        );
      }
      ...
      if (result.recoveryTime !== null) {
        addLogEntry(
          `Pod recovered in ${result.recoveryTime.toFixed(2)} seconds`,
          'success'
        );
...
function renderLogs() {
  ...
  logEntries.innerHTML = activityLog
    .map(
      (log) => `
